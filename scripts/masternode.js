...
export default class Masternode {
	...
	if (!/^(https?):\/\/[^\s/$.?#][^\s]*[^\s/.]\.[^\s/.][^\s]*[^\s.]$/.test(url)) {
	    return { ok: false, err: "invalid_url" };
	}

	if (nPayments < 1 ||  nPayments > cChainParams.current.maxPaymentCycles) {
	    return { ok: false, err: "invalid_payment_count" };
	}

	if (monthlyPayment <  10 * COIN || monthlyPayment * nPayments > cChainParams.current.maxPayment) {
