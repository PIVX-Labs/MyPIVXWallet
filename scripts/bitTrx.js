...
export default class bitjs {
	...
	outputs = [];
	locktime = 0;
	addinput({txid, index, script, sequence, path = getDerivationPath()}) {
	    const o = {};
	...
	transactionHash(index, sigHashType) {
		...
		buffer = new Uint8Array([...buffer, ...(bitjs.numToBytes(parseInt(shType), 4))]);
	...
	async signinput(index, masterKey, sigHashType, txType = 'pubkey') {
	    const strWIF = await masterKey.getPrivateKey(this.inputs[index].path);
            const bPubkeyBytes = hexToBytes(deriveAddress({pkBytes: parseWIF(strWIF), output: "COMPRESSED_HEX"}));
	    const nSigHashType = sigHashType || 1;
	...
	serialize() {
	    ...
	    buffer = [
		...buffer,
		...bitjs.numToVarInt(this.outputs.length),
	    ];
