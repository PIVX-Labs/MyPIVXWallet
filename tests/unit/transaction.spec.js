...
testVector = testVector.map(([tx, txid, hex, wif]) => [new Transaction({
    version: tx.version,
    vin: tx.vin.map(input => new CTxIn({
    ...
    vout: tx.vout.map(output => new CTxOut({
...
describe('transaction tests', () => {
    ...
    it.each(testVector.filter(t => t[3]))('signs correctly ($txid)', async (tx, _, hex, inputs) => {
